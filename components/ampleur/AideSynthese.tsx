'use client'
import { AccordionTitle, Card, CTA, CTAWrapper } from '@/components/UI'
...
export default function AideSynthese({
  ...
  const groupedByType = searchParams["ampleur.synthèse"]
                          ...
                          .reduce((acc, { rule, dottedName }) => {
                            ...
                            acc[type] += engine.setSituation(situation).evaluate(dottedName + " . montant").nodeValue;
                            return acc;
                          }, {});
  ...
  return (
    <CustomQuestionWrapper>
        ...
        { Object.entries(groupedByType).map(([type, montant]) => (
            <div key={type}>
              ...
              >
                ...
                <span>
                  <strong>{formatValue(montant)} €</strong><br />
                  <small>{type == "remboursement" ? 
                            "remboursés" :
                            type == "prêt" ? "de prêt à taux zéro" :
                            type == "exonération fiscale" ? "d'exonération fiscale" : 
                            type == "avance" ? "versés avant travaux" : ""}</small>
                </span>
              </PictoTypeAide>
            </div>
          ))
      ...
      <p>
        Pour ne pas perdre votre synthèse, sauvegardez-la en copiant le lien suivant :
      </p>
      <p css={`text-wrap: wrap;word-break: break-all;`}>{window.location.href}</p>
      <CTAWrapper $justify="center">
        <CTA
          css={`padding: 1rem 4rem;`}
      ...
      <div css={`margin-bottom 1rem;`}>
        { searchParams["ampleur.synthèse"]?.split(',').map((dottedNameUnparsed) => {
            let dottedName = decodeDottedName(dottedNameUnparsed).replace(/"/g, '')
            const rule = rules[dottedName]
            const marque2 = rule['complément de marque']
            const title = rule.marque + (marque2 ? ' - ' + uncapitalise0(marque2) : '')
            const AideComponent = correspondance[dottedName]

            return (<section key={dottedName}>
              <AccordionTitle
